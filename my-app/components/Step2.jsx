...
export default function Home({ setCurrent, setScores, setFFns, subPrs, pairwiseComparisons, updateTable }) {
  const numberOfTables = 3
  const [numberOfPrs, setNumberOfPrs] = useState(subPrs.length);
  const [tableData, setTableData] = useState(pairwiseComparisons || {});
  ...
  useEffect(() => {
    if (!pairwiseComparisons) { // Only create empty table data if pairwiseComparisons doesn't exist
      setTableData(createEmptyTableData(numberOfPrs));
    } else {
      handleButtonClick(); // If pairwiseComparisons exists, trigger handleButtonClick to process it
    }
  }, []);
  ...
  const handleInputChange = (value, rowIdx, colIdx, tableIdx) => {
    console.log("value, rowIdx, colIdx, tableIdx",value, rowIdx, colIdx, tableIdx);
  ...
  const handleButtonClick = () => {
    ...
    console.log("scoreIndexes",scoreIndexes);
    setFFns(ffnValues);
    setScores(scoreIndexes);

    for (let i = 0; i < numberOfTables; i++) {
      axios
        .post(`${baseURL}/cr`, scoreIndexes[`table_${i}`])
        .then((res) => {
          console.log("res.data",res.data);
          setConsistentMatrices((prevState) => [...prevState, res.data]);
        })
  ...
  const handleSave = () => {
    updateTable(tableData,"pairwiseComparisons")
    ...
    <div className="w-full p-10">
      <div className="font-bold text-3xl uppercase flex items-center gap-2">
        ...
        <span className="text-sm font-normal text-muted-foreground">
          ( Pairwise comparison Between sub Rdrs )
        </span>
      ...
      {/* <div className="flex gap-5 items-center">
        <div>Number of subPrs</div>
        <Input
          value={numberOfPrs}
          type="number"
          className="w-20"
          onChange={(e) => setNumberOfPrs(e.target.value)}
        />
      </div> */}
              ...
              <tr>
                <th
                  className={`border border-gray-300 p-2 ${
                    consistentMatrices[tableIdx] == "Inconsistent Matrix" &&
                    "border-red-500 bg-red-100"
                  } ${
                    consistentMatrices[tableIdx] == "Consistent Matrix" &&
                    "border-green-500 bg-green-100"
                  }`}
                ></th>
                {createArray(numberOfPrs).map((col, colIdx) => (
                  <th
                    className={`border border-gray-300 p-2 ${
                      consistentMatrices[tableIdx] == "Inconsistent Matrix" &&
                      "border-red-500 bg-red-100"
                    ...
                    key={colIdx}
                  >{subPrs[colIdx].name}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {createArray(numberOfPrs).map((_, rowIdx) => (
                <tr key={rowIdx}>
                  <th
                    className={`border border-gray-300 p-2 ${
                      consistentMatrices[tableIdx] == "Inconsistent Matrix" &&
                  ...
                  {createArray(numberOfPrs).map((_, colIdx) => (
                    <td
      ...
      <div className="flex gap-2">
        <Button onClick={()=>setCurrent(1)}>Prev</Button>
