...
export default function Home({
  ...
  const [numberOfSdrs, setNumberOfSdrs] = useState(sdrs1.length);
  const [tableData, setTableData] = useState(sdr1PrRelation || {});
  ...
  console.log("step5 table",tableData);

  useEffect(() => {
    if (!sdr1PrRelation) { // Only create empty table data if sdr1PrRelation doesn't exist
      setTableData(createEmptyTableData(numberOfSdrs));
    } else {
      handleButtonClick(); // If sdr1PrRelation exists, trigger handleButtonClick to process it
    }
  }, [numberOfSdrs]);
  ...
  const handleSave = () => {
    updateTable(tableData,"sdr1PrRelation")
    ...
    <div className="">
      <div className="font-bold text-3xl uppercase flex items-center gap-2">
        ...
        <span className="text-sm font-normal text-muted-foreground">
          ( Relationship Between PMs & sub RDRs )
        </span>
      ...
      <div className="flex gap-2 mt-5">
        <Button onClick={() => setCurrent(4)}>Prev</Button>
